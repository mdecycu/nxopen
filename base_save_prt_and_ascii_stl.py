...
def main() : 
    ...
    fileName = current_directory + "\\base"
    partName = fileName + ".prt"
    ...
    sTLCreator1.OutputFile = current_directory + "\\base_binary.stl"
    ...
    stlFilename = current_directory + "\\base_ascii_stl.txt"
    
    if os.path.exists(stlFilename):
        # 若希望存檔的零件存在, 則先刪除該零件檔案
        os.remove(stlFilename)
    
    sTLCreator1.OutputFile = stlFilename
