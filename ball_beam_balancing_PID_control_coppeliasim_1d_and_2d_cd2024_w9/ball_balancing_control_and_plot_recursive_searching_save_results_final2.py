...
for simulation_count in range(max_simulations):
    ...
    # 輸出當前控制參數
    #print("Simulation", simulation_count, "控制參數:", (kp_x, ki_x, kd_x, kp_y, ki_y, kd_y))
    # 輸出當前控制參數，採用指定的格式
    print("# PID control constants for x and y coordinates")
    print(f"kp_x = {kp_x:.4f}  # Proportional gain for x")
    print(f"ki_x = {ki_x:.4f}  # Integral gain for x")
    print(f"kd_x = {kd_x:.4f}  # Derivative gain for x")
    print("")
    print(f"kp_y = {kp_y:.4f}  # Proportional gain for y")
    print(f"ki_y = {ki_y:.4f}  # Integral gain for y")
    print(f"kd_y = {kd_y:.4f}  # Derivative gain for y")
    print("-" * 30)  # 添加分隔線
